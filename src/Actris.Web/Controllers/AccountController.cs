...
namespace Actris.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [Authorize]
        public PartialViewResult UserHeader(bool noSideBar = false)
        {
            ...
            if (noSideBar)
            {
                return PartialView("Layout/_HeaderNoSideBar", user);
            }
            else
            {
        ...
        private List<UserMenu> dummyMenus()
        {
            var menu = new List<UserMenu>
            {
                ...
                new UserMenu
                {
                    ...
                    {
                        new UserMenu{Caption= "Create ACTRIS",  Link= "/ActionTrackingList", Icon="fa-solid fa-plus"},
                        new UserMenu{Caption= "Task List",  Link= "/TaskList/NeedAction", Icon="fa-solid fa-list-check"},
                        new UserMenu{Caption= "Action Tracking Report",  Link= "/ActionTrackingReport", Icon="fa-regular fa-clipboard"}
                    }
                },
                new UserMenu
                {
                    ...
                    Child = new List<UserMenu>()
                    {
                        new UserMenu{Caption= "ACT Source",  Link= "MasterData/ActionTrackingSource", Icon="fa-regular fa-folder"},
                        new UserMenu{Caption= "Priority Level",  Link=  "MasterData/CorrectiveActionPriority", Icon="fa-regular fa-folder"},
                        new UserMenu{Caption= "Overdue Impact",  Link=  "MasterData/OverdueImpact", Icon="fa-regular fa-folder"},
                    }
                },
        ...
        private void replaceRootUrl(List<UserMenu> userMenus)
        {
            ...
            foreach (var menu in userMenus)
            {
                if (menu.Link != null && !menu.Link.Contains("http"))
                {
                    ...
                    menu.Link = menu.Link.Replace("//PHE_HSSE_DEV", "/PHE_HSSE_DEV");
                    menu.Link = menu.Link.Replace("//phe_hsse_dev", "/PHE_ACTRIS");
                }
