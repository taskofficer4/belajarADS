...
namespace Actris.Infrastructure.Services
{
   public class BaseCrudService<TDto, TGridModel> : ICrudService<TDto, TGridModel>
   {
      ...
      public virtual Task<Paged<TGridModel>> GetPaged(GridParam param)
      {
      ...
      public virtual async Task Create(TDto model)
      {
      ...
      public async Task<int> Count(string currentUser = null)
		{
         return await _repo.Count(currentUser);
      }
   }
}
