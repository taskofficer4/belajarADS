...
namespace Actris.Infrastructure.EntityFramework.Repositories
{
...

	{
		...
		public virtual async Task<int> Count(string currentUser = null)
		{
			using (var connection = OpenConnection())
			{
				var count = 0;
				if (!string.IsNullOrEmpty(_crudQuery.CountQuery))
				{
					var query = _crudQuery.CountQuery;
					if (_crudQuery.SelectPagedQueryAdditionalWhere != null)
					{
						query  += " WHERE " + _crudQuery.SelectPagedQueryAdditionalWhere.Replace("@CurrentUser", currentUser);
					}
					count = await connection.QueryFirstAsync<int>(query);
				}
				return count;
			}
		}
